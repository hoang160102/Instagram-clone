  ...
  <teleport to="body">
    ...
    >
      <div class="container rounded-lg bg-white w-[430px] h-[430px]">
        ...
        <div
          v-if="user"
          class="list-users h-[305px] overflow-y-scroll mt-4 px-5"
        >
          <div
            v-for="user in listFollowers"
            :key="user.id"
          ...
          >
            <div class="info-user flex">
              ...
              <div class="ml-4 text-user">
                <div class="text-sm font-semibold username">
                  {{ user.username }}
                </div>
            ...
            <div v-if="auth.currentUser.uid !== user.id" class="follow">
              <button v-if="isFollowing" class="text-sm py-2 rounded-lg px-5 bg-gray-200 font-semibold">
                Following
              </button>
              <button v-else class="text-sm rounded-lg py-2 px-5 bg-sky-500 text-white font-semibold">
                Follow
              </button>
            </div>
          </div>
...
export default {
  setup() {
    ...
    const listFollowers = computed(() => {
      const followersUsers = user.value.followers;
      const infoFollowersUsers = followersUsers.map((follow) => {
        return allUsers.value.filter((user) => {
          return user.id === follow;
        });
      });
      return infoFollowersUsers.flat();
    });
